...
- (void)removeAwesomeIcon:(FAIcon)icon beforeTitle:(BOOL)before {
    ...
    NSString *newTitle;
    if (before) {
        newTitle = [currentTitle substringFromIndex:currentTitle.length - (title.length + 1)];
    }
    else
        newTitle = [currentTitle substringToIndex:currentTitle.length - (title.length + 2)];
...
- (void)setup {
    ...
    CGSize size = [self.currentTitle sizeWithAttributes:@{NSFontAttributeName: font}];
